...
const double Casper::em_maxprec = 0.0001;
...
double Casper::psi(double x, double T)
{
...
double Casper::prob(int fs, int fe, int bs, int be, int* pos, double T)
{
	...
	for (double l = fraglen_prob->MinimumX; l <= fraglen_prob->MaximumX; l++)
	{
		...
		double punc = (psi(rb, T) - psi(lb, T)) / psi(mb, T);
...
void Casper::setDistributions(DiscreteDF* fraglendist)
{
