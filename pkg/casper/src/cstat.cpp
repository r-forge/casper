...
double vari(int *x, int lim, int unb) 
{
    int i;
    double value;
    for(i=0,value=0; i<=lim; i++) { value += pow(x[i],2.0) / (1.0+lim); }
...
double wvari(int *x, int lim, double *w) 
{
    int i;
    double value, wtot;
    for(i=0,value=0,wtot=0; i<=lim; i++) { value += w[i]*pow(x[i],2.0); wtot += w[i]; }
...
void inv_posdef(double **a, int n, double **aout, bool *posdef) {
...
void inv_posdef_upper(double **a, int n, double **aout, bool *posdef) {
  /* Inverse of a symmetric, positive definite matrix a[1..n][1..n] using Cholesky decomposition
     Result is returned in aout 
...
void invdet_posdef(double **a, int n, double **aout, double *det_a, bool *posdef) {
...
double ddirichlet(double *w, double *alpha, int *p, int *logscale) {
...
/* Result is stored in y[1..n]. mu is the location parameter, chols is the Cholesky decomposition
   ...
   check it 
   Example: choldc(s,n,chols,posdef); //compute cholesky decomposition
...
double mnorm(double order, double m, double sd) {
  ...
  if (order == 0.0) {   /* Trivial case */
    ...
    for (i=0, res=0; i<=n/2; i++) { temp1= i+1; temp2= order-2*i+1; res+= pow(sd,2*i) * pow(m,n-2*i) / (pow(2.0,i) * exp(gamln(&temp1)) * exp(gamln(&temp2))); }
    temp1= order+1;
