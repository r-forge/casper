...
/*
 *  hash_update() - Update an entry in the hash table.  If it doesn't exist return
 * HASH_FAIL.
 *
 ...
 */
VMDEXTERNSTATIC int hash_update(hash_t *tptr, const char *key, int data) {
  int tmp;
  hash_node_t *node;
  int h;
...
VMDEXTERNSTATIC int hash_insert(hash_t *tptr, const char *key, int data) {
 ...
 /* check to see if the entry exists */
  ...
  while (tptr->entries>=HASH_LIMIT*tptr->size) rebuild_table(tptr);
