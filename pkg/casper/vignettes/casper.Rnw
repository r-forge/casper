...
(\url{ftp://ftp.sanger.ac.uk/pub/gencode/rgasp/RGASP1/inputdata/human_fastq}).
...
For instance, gene 4 has 3 annotated transcripts,
gene 6 has 1 transcript etc.
...
When setting \texttt{relativeExpr} to \texttt{TRUE}, \texttt{calExp} returns relative
...
When setting \texttt{relativeExpr} to \texttt{FALSE},
absolute expression estimates in reads per kilobase per million (RPKM)
are returned instead.

\footnotesize
<<relIsoformExpr>>=
eset <- calcExp(distrs=distrs, genomeDB=hg19DB, pc=pc, readLength=75, relativeExpr=FALSE)
head(exprs(eset))
@ 
\normalsize
...
Figure \ref{fig:plotprior1} displays these probabilities for $E=2,\ldots,5$,
and the corresponding observed proportion in the annotated genome \texttt{hg19DB}.

\footnotesize
<<plotprior1>>=
plotPriorAS(mprior, type='nbVariants',exons=2:5)
@ 
\normalsize
...
\footnotesize
<<plotprior2>>=
plotPriorAS(mprior, type='nbExons',exons=2:5)
@ 
\normalsize
...
<<calcDenovo>>=
fit1 <- calcDenovo(distrs=distrs, genomeDB=hg19denovo, pc=pcnovo, readLength=75, geneid="4")
fit1
fit1[['4']]
@ 
